...
const WatchPage = (): JSX.Element => {
  ...
  // Mock authentication state for debugging (can be replaced with Clerk later)
  ...
  const handleEpisodeComplete = async (episodeIndex: number) => {
    if (!watchedEpisodes.has(episodeIndex)) {
      ...
      if (eligibleNFT && isConnected && address) {
        ...
        try {
          const videoId = `${animeId}-episode-${episodeId}`;
          const result = await solanaService.mintWatchNFT(
            { publicKey: address, sendTransaction: window.solana?.signAndSendTransaction },
            videoId
          );
          ...
          if (result.success) {
            ...
            if (result.signature) {
              setTimeout(() => {
                toast(`Transaction: ${result.signature?.slice(0, 8)}...`, {
                  duration: 3000,
                  action: {
                    label: 'View',
                    onClick: () => window.open(solanaService.getTransactionUrl(result.signature!), '_blank')
                  }
                });
              }, 1000);
            }
          ...
          toast.error('Failed to mint NFT reward. Please try again.');
        }
